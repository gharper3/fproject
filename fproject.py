...
parent = st.radio("Are you a parent of a child under 18 living in your home?", options=[1, 2, 8, 9], format_func=lambda x: "Yes" if x == 1 else "No" if x == 2 else "Don't know" if x == 8 else "Refused")
gender = st.radio("Gender", options=list(gender_labels.keys()), format_func=lambda x: gender_labels[x])
age = st.slider("Age (up to 97+)", min_value=1, max_value=97, value=30, step=1)
